...
const RECTANGLE_ID_PATTERN = /^rect-[a-f0-9]{10}$/;

const gameIdSchema = z.string().trim().regex(UUID_PATTERN, 'Game ID must be a valid UUID.');
const rectangleIdSchema = z.string().trim().regex(RECTANGLE_ID_PATTERN, 'Rectangle ID is malformed.');
...
const selectRectangleSchema = z.object({ rectangleId: rectangleIdSchema });
const placeRectangleSchema = z.object({
  rectangleId: rectangleIdSchema,
...
const FIELD_ERROR_CODES = {
  ...
  rectangleId: 'INVALID_RECTANGLE_ID',
